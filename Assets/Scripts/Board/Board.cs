...
public class Board
{
        ...
        List<NormalItem.eNormalType> allTypes = new List<NormalItem.eNormalType>
    {
        ...
        // Kiểm tra tổng số ô phải chia hết cho 3
        if (totalCells % 3 != 0)
        {
            Debug.LogError("Total cells must be a multiple of 3!");
            return;
        }
        ...
        // Đảm bảo mỗi loại có ít nhất 3 item (để tránh kẹt game)
        int baseCount = (totalCells / allTypes.Count) / 3 * 3; // Chia đều và đảm bảo chia hết cho 3
        int remainingItems = totalCells - (baseCount * allTypes.Count);

        foreach (var type in allTypes)
        {
            itemCounts[type] = baseCount;
        }

        // Phân bổ số dư sao cho mỗi loại vẫn chia hết cho 3
        int i = 0;
        while (remainingItems > 0)
        {
            itemCounts[allTypes[i % allTypes.Count]] += 3; // Thêm theo nhóm 3
            remainingItems -= 3;
            i++;
        }
        ...
        foreach (var kvp in itemCounts)
        {
            ...
            {
                itemPool.Add(kvp.Key);
            }
        }

        // Xáo trộn danh sách để tránh sắp xếp theo mẫu dễ đoán
    ...
    //{
        ...
        //{
        //    Debug.LogError("One of the cells is null! Cannot swap.");
        //    return;
        //}
        ...
        //{
        //    Debug.LogError("One of the items is null! Cannot swap.");
        //    return;
        //}
